...
@section('content')
    <div
        class="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-8"
        x-data="{
        ...
        >
            ...
            <div class="md:col-span-2">
                <div class="bg-white border border-slate-200/80 rounded-3xl overflow-hidden shadow-xs">
                    ...
                    <div class="p-6 space-y-3">
                        ...
                        >
                            <div
                                :class="selectedMethod === 'wallet' ? 'scale-100' : 'scale-0'"
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        <p class="text-sm font-black text-slate-900">
                                            {{ __('messages.wallet_payment') }}
                                        </p>
                                        <span
                                            class="px-2 py-0.5 bg-emerald-100 text-emerald-800 text-[9px] font-black rounded-md uppercase tracking-wider"
                                        >
                                            {{ __('messages.buy_one_click') }}
                                        </span>
                                    </div>
                                    <p class="text-xs text-slate-400 font-bold mt-0.5">
                                        {{ __('messages.current_balance') }}:
                                        <strong class="{{ $hasEnough ? 'text-emerald-600' : 'text-rose-600' }}">
                                            {{ number_format($userBalance, 0, ',', '.') }} đ
                                        </strong>
                                        @if (! $hasEnough)
                                            <span class="text-rose-600 font-black ml-1">
                            ...
                            @if (! $hasEnough)
                                <a
                                    ...
                                    class="px-3 py-1.5 bg-orange-500 hover:bg-orange-600 text-white font-black text-[10px] rounded-xl shadow-xs transition-colors shrink-0 uppercase tracking-wider"
                                >
                        ...
                        {{-- SePay / VietQR --}}
                        @php
                            $sepayConfigured = app(\App\Services\SePayService::class)->isConfigured();
                        @endphp

                        <label
                            @click="{{ $sepayConfigured ? "selectedMethod = 'sepay'" : '' }}"
                        ...
                        >
                            <div
                                :class="selectedMethod === 'sepay' ? 'scale-100' : 'scale-0'"
                                ...
                                <div>
                                    ...
                                    @if (! $sepayConfigured)
                                        <p class="text-xs text-rose-500 font-bold">
                                            {{ __('messages.gateway_not_configured') }}
                                        </p>
                                    @else
                                        <p class="text-xs text-slate-400 font-bold">
                                            {{ __('messages.vietqr_instruction_short') }}
                                        </p>
                                    @endif
                                </div>
                            </div>
                            <div
                                class="px-2.5 py-1 bg-gradient-to-tr from-primary-600 to-amber-500 text-white font-black text-[9px] rounded-md uppercase tracking-wider"
                            >
                                SePay
                            </div>
                        </label>
                        ...
                        <label
                            @click="{{ $stripeConfigured ? "selectedMethod = 'stripe'" : '' }}"
                        ...
                        >
                            <div
                                :class="selectedMethod === 'stripe' ? 'scale-100' : 'scale-0'"
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    @else
                                        <p class="text-xs text-slate-400 font-bold">
                                            {{ __('messages.stripe_card_types') }}
                                        </p>
                                    @endif
                                </div>
                            </div>
                            <div
                                class="px-2.5 py-1 bg-indigo-600 text-white font-black text-[9px] rounded-md uppercase tracking-wider"
                            >
                                Stripe
                            </div>
                        </label>
            ...
            <div class="bg-white border border-slate-200/80 rounded-3xl overflow-hidden shadow-xs sticky top-20">
                <div class="p-6 space-y-5">
                    <h3
                        class="text-sm font-black text-slate-900 uppercase tracking-tight border-b border-slate-100 pb-3"
                    >
                    ...
                    <div
                        class="border-t border-dashed border-slate-200 pt-3 space-y-2 text-xs font-bold text-slate-500"
                    >
                    ...
                    <div
                        class="flex justify-between text-base font-black text-slate-900 border-t border-slate-100 pt-3"
                    >
                    ...
                    <button
                        type="submit"
                        class="btn-primary w-full py-3.5 rounded-xl font-black text-sm flex items-center justify-center gap-2 shadow-lg shadow-orange-500/20 cursor-pointer uppercase tracking-wider active:scale-[0.98] transition-transform"
                    >
                ...
                <div class="flex items-center justify-center gap-2 border-t border-slate-100 bg-slate-50/50 px-4 py-3">
                    ...
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        {{ __('messages.secure_payment_badge') }}
                    </p>
                </div>
            </div>
        </form>
