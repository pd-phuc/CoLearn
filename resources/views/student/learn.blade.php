...
<body class="h-full font-sans antialiased bg-slate-100 text-slate-900 flex flex-col overflow-hidden"
      x-data="{
          sidebarOpen: true,
          ...
          async toggleComplete(lessonId) {
              try {
                  const res = await fetch('/learn/lessons/' + lessonId + '/toggle-complete', {
                      method: 'POST',
                      headers: {
                  ...
                  if (res.ok) {
                      ...
                      if (data.completed) {
                          ...
                          this.completedIds = this.completedIds.filter(id => id !== lessonId);
                      }
                  }
              } catch (e) {
                  console.error(e);
    ...
    <header class="bg-white border-b border-slate-200 shrink-0 z-30 shadow-xs relative">
        <div class="h-16 px-4 sm:px-6 flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="truncate max-w-md">
                    ...
                    <p class="text-xs text-slate-500 font-medium truncate">{{ $course->category->name }} &bull; {{ $course->teacher->name ?? __('messages.default_instructor_name') }}</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="hidden sm:flex items-center gap-2 bg-slate-100 px-3.5 py-1.5 rounded-full border border-slate-200 text-xs font-bold text-slate-700">
                    <span>{{ __('messages.progress_label') }}:</span>
                    <span class="text-orange-600 font-extrabold">
                        <span x-text="completedCount"></span>/<span x-text="totalCount"></span> (<span x-text="progressPercent + '%'"></span>)
                    </span>
                </div>
        ...
        <div class="w-full bg-slate-200 h-1 relative overflow-hidden">
            <div class="bg-gradient-to-r from-orange-500 to-amber-500 h-full transition-all duration-500"
    ...
    <div class="flex-1 flex overflow-hidden relative">
        ...
        <div class="flex-1 overflow-y-auto flex flex-col justify-between p-4 sm:p-6 space-y-6 bg-slate-100">
            <div class="max-w-5xl mx-auto w-full space-y-6">
                ...
                <div class="aspect-video w-full bg-black rounded-2xl overflow-hidden border border-slate-200 shadow-xs relative">
                    ...
                    @else
                        ...
                        <div class="w-full h-full bg-white p-8 flex flex-col justify-center items-center text-center space-y-4">
                            ...
                            <p class="text-sm text-slate-600 max-w-lg leading-relaxed">{{ $lesson->content ?: __('messages.document_lesson_notice') }}</p>
                        </div>
                    @endif
                </div>

                <!-- Interactive Action Toolbar Card -->
                <div class="bg-white border border-slate-200/80 rounded-2xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xs">
                    <div>
                        ...
                        <div class="flex items-center gap-3 text-xs text-slate-500 font-semibold mt-1">
                            ...
                            <span>&bull;</span>
                            <span>{{ sprintf('%02d:%02d', floor($lesson->duration / 60), $lesson->duration % 60) }} {{ __('messages.minutes_short') }}</span>
                            @if($lesson->is_free_preview)
                                <span class="px-2 py-0.5 rounded-md bg-orange-100 text-orange-700 text-[10px] font-extrabold uppercase tracking-wider">{{ __('messages.free_preview_badge') }}</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 w-full sm:w-auto">
                        ...
                        <button @click="toggleComplete('{{ $lesson->id }}')"
                            ...
                            <svg class="w-4 h-4 shrink-0" :class="isCompleted ? 'text-white' : 'text-slate-500'" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7" />
                            </svg>
                            <span x-text="isCompleted ? '{{ __('messages.completed_status') }}' : '{{ __('messages.mark_as_completed') }}'"></span>
                        </button>

                        <!-- Next Lesson Button -->
                        @if($nextLesson)
                            <a href="{{ route('learn.show', ['course' => $course->slug, 'lesson' => $nextLesson->id]) }}" 
                ...
                @if($lesson->content || $lesson->document_path)
                    <div class="bg-white border border-slate-200/80 rounded-2xl p-6 space-y-4 shadow-xs">
                        ...
                        @if($lesson->document_path)
                            <div class="pt-2">
                                <a href="{{ asset($lesson->document_path) }}" download class="inline-flex items-center gap-2 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-orange-600 rounded-xl text-xs font-bold transition-colors border border-slate-200">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <aside x-show="sidebarOpen" 
            ...
            <div class="p-4 border-b border-slate-200 flex items-center justify-between bg-white">
                <div>
                    <h3 class="text-sm font-extrabold text-slate-900">{{ __('messages.course_curriculum_title') }}</h3>
                    <p class="text-xs text-slate-500 font-medium mt-0.5">
                        <span x-text="completedCount"></span> / <span x-text="totalCount"></span> {{ __('messages.lessons_completed_sub') }}
                    </p>
                </div>
                    ...
                    <div x-data="{ open: true }">
                        ...
                        <button @click="open = !open" 
                            ...
                            <div>
                                <h4 class="text-xs font-extrabold text-slate-800 uppercase tracking-wider">
                                    {{ __('messages.section_num', ['num' => $sectionIndex + 1]) }}: {{ $section->title }}
                                </h4>
                        ...
                        <div x-show="open" class="divide-y divide-slate-100 bg-white">
                            @foreach($section->lessons as $item)
                                ...
                                @endphp
                                <a href="{{ $canAccess ? route('learn.show', ['course' => $course->slug, 'lesson' => $item->id]) : '#' }}"
                                   class="p-3.5 flex items-start gap-3 transition-colors {{ $isActive ? 'bg-orange-50/80 border-l-4 border-orange-500 text-orange-950 font-bold' : 'hover:bg-slate-50 text-slate-700' }} {{ ! $canAccess ? 'opacity-50 cursor-not-allowed' : '' }}">
                                    ...
                                    <div class="mt-0.5 shrink-0">
                                        ...
                                        <template x-if="!completedIds.includes('{{ $item->id }}')">
                                            <div>
                                                ...
                                                @else
                                                    <div class="w-5 h-5 rounded-full border border-slate-300 text-slate-400 flex items-center justify-center">
                                                        <svg class="w-3 h-3 fill-current ml-0.5" viewBox="0 0 24 24">
                                                            <path d="M8 5v14l11-7z"/>
                                                        </svg>
                                                    </div>
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-xs font-semibold truncate leading-tight {{ $isActive ? 'text-orange-600 font-extrabold' : 'text-slate-800' }}">
                                            {{ $item->title }}
                                        </p>
                                        <div class="flex items-center gap-2 text-[10px] text-slate-500 font-medium mt-1">
                                            <span>{{ sprintf('%02d:%02d', floor($item->duration / 60), $item->duration % 60) }} {{ __('messages.minutes_short') }}</span>
                                            @if($item->is_free_preview && ! $isEnrolled)
                                                <span class="text-orange-600 font-bold">{{ __('messages.free_preview_badge') }}</span>
                                            @endif
                                        </div>
                                    </div>
                                </a>
