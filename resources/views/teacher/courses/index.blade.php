...
@section('teacher-content')
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-xs flex flex-col md:flex-row gap-4">
            <form
                method="GET"
                action="{{ route('teacher.courses.index') }}"
            ...
            >
                <div class="relative flex-1">
                    <input
                        ...
                        placeholder="{{ __('teacher.search_course_placeholder') }}"
                        class="w-full pl-10 pr-4 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-blue-500 focus:outline-none font-medium"
                    />
                ...
                <select
                    ...
                    class="px-4 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-blue-500 focus:outline-none font-bold text-slate-700"
                >
                    <option value="">{{ __('teacher.all_statuses') }}</option>
                    <option value="draft" {{ request('status') === 'draft' ? 'selected' : '' }}>
                        {{ __('teacher.status_draft') }}
                    </option>
                    <option value="pending_review" {{ request('status') === 'pending_review' ? 'selected' : '' }}>
                        {{ __('teacher.status_pending_review') }}
                    </option>
                    <option value="published" {{ request('status') === 'published' ? 'selected' : '' }}>
                        {{ __('teacher.status_published') }}
                    </option>
                    <option value="archived" {{ request('status') === 'archived' ? 'selected' : '' }}>
                        {{ __('teacher.status_archived') }}
                    </option>
                </select>

                <button
                    type="submit"
                    class="px-4 py-2 bg-slate-900 text-white rounded-xl text-xs font-bold hover:bg-slate-800 transition-colors"
                >
                    {{ __('teacher.filter') }}
                </button>
            </form>
                        ...
                        @forelse ($courses as $course)
                            <tr class="hover:bg-slate-50/60 transition-colors">
                                    ...
                                    <div class="flex items-center gap-3.5 min-w-[240px]">
                                        @if ($course->thumbnail)
                                            <img
                                                ...
                                                alt=""
                                                class="w-14 h-10 object-cover rounded-lg border border-slate-200 shrink-0"
                                            />
                                        @else
                                            <div
                                                class="w-14 h-10 bg-slate-100 rounded-lg flex items-center justify-center text-slate-400 font-bold shrink-0 text-[10px]"
                                            >
                                                {{ __('teacher.no_image') }}
                                            </div>
                                        @endif
                                        <div class="min-w-0">
                                            <a
                                                ...
                                                class="font-extrabold text-slate-900 hover:text-blue-600 block truncate"
                                            >
                                                {{ $course->title }}
                                            </a>
                                ...
                                <td class="py-4 px-4 whitespace-nowrap">
                                    @if ($course->price == 0)
                                        <span class="font-black text-emerald-600 uppercase">
                                            {{ __('teacher.free') }}
                                        </span>
                                    @else
                                    ...
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        @if ($course->status === 'draft')
                                            <form
                                                ...
                                                method="POST"
                                                onsubmit="
                                                    return confirm('{{ __('teacher.submit_for_review_confirm') }}');
                                                "
                                            >
                                                @csrf
                                            ...
                                            <form
                                                ...
                                                method="POST"
                                                onsubmit="return confirm('{{ __('teacher.delete_draft_confirm') }}');"
                                            >
                                                @csrf
                                                @method('DELETE')
                                                <button
