                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td class="py-4 px-4 whitespace-nowrap">
                                @if($course->status === 'published')
                                    <span class="px-2.5 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200/60 font-bold rounded-lg">{{ __('teacher.status_published') }}</span>
                                @elseif($course->status === 'pending_review')
                                    <span class="px-2.5 py-1 bg-blue-50 text-blue-700 border border-blue-200/60 font-bold rounded-lg">{{ __('teacher.status_pending_review') }}</span>
                                @elseif($course->status === 'draft')
                                    <span class="px-2.5 py-1 bg-amber-50 text-amber-700 border border-amber-200/60 font-bold rounded-lg">{{ __('teacher.status_draft') }}</span>
                                @else
                                    <span class="px-2.5 py-1 bg-slate-100 text-slate-600 font-bold rounded-lg">{{ $course->status }}</span>
                                @endif
                            </td>
