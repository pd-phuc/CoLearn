...
@section('teacher-content')
    <div class="space-y-6" x-data="{ activeTab: 'curriculum' }">
        ...
        <div
            class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white rounded-3xl p-6 border border-slate-200/80 shadow-xs"
        >
            <div>
                <a
                    href="{{ route('teacher.courses.index') }}"
                    class="inline-flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-blue-600 mb-2"
                >
            ...
            <div class="flex items-center gap-3">
                ...
                @if ($course->status === 'draft')
                    <form
                        action="{{ route('teacher.courses.submit-review', $course) }}"
                        method="POST"
                        onsubmit="return confirm('{{ __('teacher.submit_for_review_confirm') }}');"
                    >
                        @csrf
            ...
            <div class="bg-rose-50 border border-rose-200 rounded-2xl p-4 text-rose-800 flex items-start gap-3">
                ...
                <div>
                    <p class="text-xs font-black uppercase tracking-wider text-rose-900">
                        {{ __('teacher.rejection_reason_notice') }}
                    </p>
        ...
        {{-- Published Course Edit Warning --}}
        @if ($course->status === 'published')
            <div class="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-amber-800 flex items-start gap-3">
                <svg
                ...
                <p class="text-xs font-bold text-amber-800">
                    {{ __('teacher.warning_edit_published') }}
                </p>
            </div>
        @endif
        ...
        <div x-show="activeTab === 'curriculum'" class="space-y-6">
            {{-- Add Section Card --}}
            <div
                class="bg-blue-50/60 border border-blue-100 rounded-3xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4"
            >
                <div>
                ...
                <form
                    action="{{ route('teacher.courses.sections.store', $course) }}"
                    method="POST"
                ...
                >
                    ...
                    <input
                        ...
                        name="title"
                        required
                        placeholder="{{ __('teacher.section_title') }}"
            ...
            <div class="space-y-4">
                @forelse ($course->sections as $index => $section)
                    ...
                    >
                        {{-- Section Header --}}
                        <div
                            class="p-4 sm:p-5 bg-slate-50/80 border-b border-slate-200/60 flex items-center justify-between gap-4"
                        >
                            <div class="flex items-center gap-3 min-w-0 cursor-pointer" @click="expanded = !expanded">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <span class="text-[11px] font-semibold text-slate-500">
                                        {{ $section->lessons->count() }} {{ __('teacher.lessons_count') }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-2 shrink-0">
                                <x-button
                                    ...
                                    size="xs"
                                    type="button"
                                    @click="addLessonOpen = !addLessonOpen"
                                    class="gap-1"
                                >
                                ...
                                <button
                                    type="button"
                                    @click="editSectionOpen = !editSectionOpen"
                                    class="p-1.5 text-slate-400 hover:text-slate-700 transition-colors"
                                >
                                    <x-icon name="pencil" size="sm" />
                                </button>

                                <form
                                    action="{{ route('teacher.sections.destroy', $section) }}"
                                    method="POST"
                                    onsubmit="return confirm('{{ __('teacher.delete_section_confirm') }}');"
                                >
                                    @csrf
                                    @method('DELETE')
                                    <button
                        ...
                        <div
                            x-show="editSectionOpen"
                            class="p-4 bg-amber-50/60 border-b border-amber-100 flex items-center gap-3"
                        >
                            <form
                                action="{{ route('teacher.sections.update', $section) }}"
                                method="POST"
                            ...
                            >
                                ...
                                <input
                                    ...
                                    value="{{ $section->title }}"
                                    required
                                    class="px-3 py-1.5 text-xs bg-white border border-amber-200 rounded-lg focus:outline-none focus:border-amber-500 font-medium flex-1"
                                />
                                <x-button
                                    variant="primary"
                                    size="xs"
                                ...
                                <x-button variant="secondary" size="xs" type="button" @click="editSectionOpen = false">
                                    {{ __('teacher.cancel') }}
                                </x-button>
                            </form>
                        </div>
                        ...
                        <div x-show="addLessonOpen" x-transition class="p-5 bg-blue-50/30 border-b border-blue-100">
                            <form
                                action="{{ route('teacher.sections.lessons.store', $section) }}"
                                method="POST"
                                enctype="multipart/form-data"
                                class="space-y-4"
                            >
                                @csrf
                                <h5 class="text-xs font-black text-blue-900 uppercase tracking-wider">
                                    {{ __('teacher.add_lesson') }}
                                </h5>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    <div class="sm:col-span-2">
                                        <label class="block text-[11px] font-bold text-slate-600 mb-1">
                                            {{ __('teacher.lesson_title') }} *
                                        </label>
                                        <input
                                            type="text"
                                            name="title"
                                            required
                                            placeholder="{{ __('teacher.lesson_title_placeholder') }}"
                                            class="w-full px-3 py-2 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:border-blue-500 font-medium"
                                        />
                                    </div>
                                    <div>
                                        <label class="block text-[11px] font-bold text-slate-600 mb-1">
                                            {{ __('teacher.lesson_type') }} *
                                        </label>
                                        <select
                                            name="type"
                                            class="w-full px-3 py-2 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:border-blue-500 font-bold text-slate-700"
                                        >
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    <div class="sm:col-span-2">
                                        <label class="block text-[11px] font-bold text-slate-600 mb-1">
                                            {{ __('teacher.video_url') }}
                                        </label>
                                        <input
                                            type="text"
                                            name="video_url"
                                            placeholder="{{ __('teacher.video_url_placeholder') }}"
                                            class="w-full px-3 py-2 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:border-blue-500 font-medium"
                                        />
                                    </div>
                                    <div>
                                        <label class="block text-[11px] font-bold text-slate-600 mb-1">
                                            {{ __('teacher.duration_minutes') }}
                                        </label>
                                        <input
                                            type="number"
                                            name="duration"
                                            min="0"
                                            value="10"
                                            class="w-full px-3 py-2 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:border-blue-500 font-bold"
                                        />
                                    </div>
                                </div>
                                ...
                                <div>
                                    <label class="block text-[11px] font-bold text-slate-600 mb-1">
                                        {{ __('teacher.lesson_content') }}
                                    </label>
                                    <textarea
                                        name="content"
                                        rows="3"
                                        placeholder="{{ __('teacher.lesson_content_placeholder') }}"
                                        class="w-full px-3 py-2 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:border-blue-500 font-medium"
                                    ></textarea>
                                </div>
                        ...
                        <div x-show="expanded" class="divide-y divide-slate-100">
                            @forelse ($section->lessons as $lIndex => $lesson)
                                <div x-data="{ editLessonOpen: false }">
                                    <div
                                        class="p-3.5 sm:px-6 flex items-center justify-between gap-3 hover:bg-slate-50/50 transition-colors"
                                    >
                                        <div class="flex items-center gap-3 min-w-0">
                                            <div
                                                class="w-7 h-7 rounded-lg bg-slate-100 text-slate-500 flex items-center justify-center text-xs font-bold shrink-0"
                                            >
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <div
                                                    class="flex items-center gap-2 text-[10px] text-slate-400 font-semibold mt-0.5"
                                                >
                                                    <span>{{ $lesson->duration }} {{ __('teacher.minutes') }}</span>
                                                    @if ($lesson->is_free_preview)
                                                        <span
                                                            class="px-1.5 py-0.5 bg-emerald-100 text-emerald-700 rounded font-bold"
                                                        >
                                                            {{ __('teacher.free_preview_badge') }}
                                                        </span>
                                                    @endif
                                                </div>
                                            </div>
                                        ...
                                        <div class="flex items-center gap-2 shrink-0">
                                            ...
                                            <button
                                                type="button"
                                                @click="editLessonOpen = !editLessonOpen"
                                                class="p-1.5 text-slate-400 hover:text-slate-700 transition-colors"
                                            >
                                                <svg
                                                    class="w-3.5 h-3.5"
                                            ...
                                            <form
                                                ...
                                                method="POST"
                                                onsubmit="return confirm('{{ __('teacher.delete_lesson_confirm') }}');"
                                            >
                                                @csrf
                                                @method('DELETE')
                                                <button
                                    ...
                                    <div
                                        x-show="editLessonOpen"
                                        x-transition
                                        class="px-5 py-4 bg-amber-50/60 border-t border-amber-100"
                                    >
                                        <form
                                        ...
                                        >
                                            ...
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <input
                                                        ...
                                                        value="{{ $lesson->title }}"
                                                        required
                                                        class="w-full px-3 py-1.5 text-xs bg-white border border-amber-200 rounded-lg focus:outline-none focus:border-amber-500 font-medium"
                                                    />
                                                </div>
                                            ...
                                            {{-- Content (shown when type is text) --}}
...
{{ $lesson->content }}</textarea>
                                            ...
                                            {{-- Document file (shown when type is document) --}}
                                            <div x-show="lessonType === 'document'">
                                                <label class="block text-xs font-bold text-slate-700 mb-1">
                                                    {{ __('teacher.document_file') }}
                                                </label>
                                                <input
                                                    type="file"
                                                    name="document_file"
                                                    class="w-full text-xs text-slate-500 file:mr-3 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-amber-100 file:text-amber-700 hover:file:bg-amber-200"
                                                />
                                            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select
                            name="category_id"
                            required
                            class="w-full px-4 py-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-blue-500 focus:outline-none font-semibold text-slate-700"
                        >
                    ...
                    <div>
                        ...
                        <select
                            ...
                            class="w-full px-4 py-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-blue-500 focus:outline-none font-semibold text-slate-700"
                        >
                            <option value="all" {{ old('level', $course->level) == 'all' ? 'selected' : '' }}>
                                {{ __('teacher.level_all') }}
                            </option>
                            <option
                                value="beginner"
                                {{ old('level', $course->level) == 'beginner' ? 'selected' : '' }}
                            >
                                {{ __('teacher.level_beginner') }}
                            </option>
                            <option
                                value="intermediate"
                    ...
                    @if ($course->thumbnail)
                        <div class="mb-3">
                            <img
                                ...
                                alt=""
                                class="w-48 h-28 object-cover rounded-xl border border-slate-200"
                            />
                        </div>
                    @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <textarea
                            ...
                            rows="4"
                            class="w-full px-4 py-3 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-blue-500 focus:outline-none font-medium"
                        >
{{ old('learning_outcomes', is_array($course->learning_outcomes) ? implode("\n", $course->learning_outcomes) : '') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            class="w-full px-4 py-3 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-blue-500 focus:outline-none font-medium"
                        >
{{ old('requirements', is_array($course->requirements) ? implode("\n", $course->requirements) : '') }}</textarea>
                    </div>
                </div>
                ...
                <div class="pt-4 border-t border-slate-100 flex justify-end">
                    <button
                        type="submit"
                        class="px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-black transition-all"
                    >
