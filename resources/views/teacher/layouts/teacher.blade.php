                ...
                <div class="flex items-center justify-between gap-4 pb-2.5 border-b border-slate-100">
                    ...
                    <div class="flex items-center gap-3.5 shrink-0">
                        ...
                        <a href="{{ route('home') }}" target="_blank" class="hidden sm:inline-flex items-center gap-2 px-3.5 py-1.5 text-sm font-bold text-slate-700 hover:text-blue-600 hover:bg-blue-50/60 rounded-xl transition-colors">
                            <svg class="w-4.5 h-4.5 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                        ...
                        <div x-data="{ open: false }" @click.outside="open = false" class="relative shrink-0">
                            <button @click="open = !open" class="flex items-center gap-2 p-1 rounded-full hover:bg-slate-100 transition-colors focus:outline-none">
                                @if(auth()->user()->avatar)
                                    <img src="{{ auth()->user()->avatar }}" alt="" class="w-9 h-9 rounded-full object-cover ring-2 ring-blue-500/30">
                                @else
                                    <div class="w-9 h-9 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-500 text-white font-bold flex items-center justify-center text-sm shadow-sm">
    ...
    <footer class="bg-slate-950 text-slate-400 border-t border-slate-800/80 mt-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 py-14">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-10">
                ...
                <div class="space-y-4">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-xl bg-blue-600 flex items-center justify-center text-white font-black text-xl shadow-md">C</div>
                        <span class="text-2xl font-black text-white">Co<span class="text-blue-500">Learn</span></span>
                    </div>
                    ...
                    <div class="flex items-center gap-3 text-xs font-bold text-slate-300">
                        <span class="px-2.5 py-1 bg-slate-900 border border-slate-800 rounded-lg">Instructor Hub</span>
                        <span class="px-2.5 py-1 bg-slate-900 border border-slate-800 rounded-lg">Teacher Portal</span>
                    </div>
                </div>

                <!-- Col 2: Teacher Quick Links -->
                <div>
                    <h3 class="text-xs font-extrabold text-white uppercase tracking-wider mb-4">{{ __('teacher.portal') }}</h3>
                    <ul class="space-y-2.5 text-sm font-medium">
                        <li><a href="{{ route('teacher.dashboard') }}" class="hover:text-blue-400 transition-colors">{{ __('teacher.dashboard') }}</a></li>
                        <li><a href="{{ route('teacher.courses.index') }}" class="hover:text-blue-400 transition-colors">{{ __('teacher.my_courses') }}</a></li>
                        <li><a href="{{ route('teacher.courses.create') }}" class="hover:text-blue-400 transition-colors">{{ __('teacher.create_course') }}</a></li>
                        <li><a href="{{ route('teacher.students.index') }}" class="hover:text-blue-400 transition-colors">{{ __('teacher.students') }}</a></li>
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-2.5 text-sm font-medium">
                        <li><a href="{{ route('courses.index') }}" class="hover:text-blue-400 transition-colors">Lập Trình Web Laravel 13</a></li>
                        <li><a href="{{ route('courses.index') }}" class="hover:text-blue-400 transition-colors">C++ & Thuật Toán Chuyên Tin</a></li>
                        <li><a href="{{ route('courses.index') }}" class="hover:text-blue-400 transition-colors">Cơ Sở Dữ Liệu PostgreSQL</a></li>
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-2 text-sm font-medium">
                        <div class="flex items-center gap-2 text-slate-300">
                            ...
                            <span>instructor-support@colearn.vn</span>
                        </div>
            ...
            <div class="border-t border-slate-800/80 mt-12 pt-6 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 gap-4">
                <p>&copy; {{ date('Y') }} CoLearn Teacher Portal. {{ __('messages.footer_rights') }}</p>
