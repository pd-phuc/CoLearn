...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full bg-slate-100/60">
    <head>
        ...
        <title>
            {{ isset($title) ? $title . ' — ' . __('teacher.portal') : __('teacher.portal') . ' — ' . __('messages.app_name') }}
    ...
    <body
        class="min-h-screen bg-gradient-to-b from-slate-100/60 via-slate-50 to-white font-sans antialiased text-slate-900 flex flex-col justify-between selection:bg-primary-500 selection:text-white"
            ...
            <header class="sticky top-3 z-50 mx-auto w-full max-w-7xl px-4 sm:px-6 select-none">
                ...
                >
                    ...
                    <div class="flex items-center justify-between gap-4 pb-2.5 border-b border-slate-100">
                        ...
                        <div class="flex items-center gap-2.5 shrink-0">
                            ...
                            >
                                <div class="relative">
                                    <div
                                        class="w-9 h-9 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-500 flex items-center justify-center text-white font-black text-xl shadow-md shadow-blue-500/20 group-hover:scale-105 transition-transform duration-200 select-none"
                                        aria-hidden="true"
                                    >
                                        C
                                    </div>
                                    <div
                                        class="absolute inset-0 rounded-xl bg-blue-500/30 blur-md opacity-0 group-hover:opacity-100 transition-opacity"
                                    ></div>
                                </div>
                                <span class="text-xl font-extrabold tracking-tight text-slate-900 select-none">
                                    Co
                                    <span class="text-blue-600">Learn</span>
                                </span>
                            </a>
                            <span
                                class="px-2 py-0.5 bg-blue-50 text-blue-700 text-[10px] font-black uppercase tracking-widest rounded-lg border border-blue-200/60 shadow-2xs select-none"
                            >
                                Teacher
                            </span>
                        </div>
                        ...
                        <div class="flex items-center gap-3.5 shrink-0">
                            ...
                            <a
                                href="{{ route('home') }}"
                                target="_blank"
                                class="hidden sm:inline-flex items-center gap-2 px-3.5 py-1.5 text-sm font-bold text-slate-700 hover:text-blue-600 hover:bg-blue-50/60 rounded-xl transition-colors"
                            >
                            ...
                            >
                                <a
                                    ...
                                    class="px-3 py-1 rounded-lg transition-all {{ app()->getLocale() === 'vi' ? 'bg-white text-blue-600 shadow-xs' : 'text-slate-500 hover:text-slate-900' }}"
                                >
                                    VI
                                </a>
                                <a
                                    href="{{ route('lang.switch', 'en') }}"
                                    class="px-3 py-1 rounded-lg transition-all {{ app()->getLocale() === 'en' ? 'bg-white text-blue-600 shadow-xs' : 'text-slate-500 hover:text-slate-900' }}"
                                >
                                    EN
                                </a>
                            </div>
                                ...
                                >
                                    ...
                                    <div class="py-1">
                                        <a
                                            ...
                                            class="flex items-center gap-2.5 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-blue-50/80 hover:text-blue-600"
                                        >
                    ...
                    <div class="pt-0.5">
                        <nav class="hidden lg:flex items-center justify-between w-full gap-2">
                            @foreach ($navItems as $item)
                                ...
                                >
                                    <svg
                                        class="w-4.5 h-4.5 shrink-0 {{ $isActive ? 'text-blue-600' : 'text-slate-400' }}"
                                        fill="none"
                                        viewBox="0 0 24 24"
                                        stroke="currentColor"
                                        stroke-width="1.8"
                    ...
                    <div
                        ...
                        x-transition
                        class="lg:hidden mt-3 pt-3 border-t border-slate-100 grid grid-cols-2 sm:grid-cols-3 gap-1.5"
                    >
                        @foreach ($navItems as $item)
                            @php
                                $isActive = request()->routeIs($item['route'] . '*');
                            ...
                            >
                                <svg
                                    class="w-4 h-4 shrink-0 {{ $isActive ? 'text-blue-600' : 'text-slate-400' }}"
                                    fill="none"
                                    viewBox="0 0 24 24"
                                    stroke="currentColor"
                                    stroke-width="1.5"
