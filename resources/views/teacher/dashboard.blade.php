...
@section('teacher-content')
    <div class="space-y-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <x-stat-card
                :label="__('teacher.total_courses')"
                :value="$totalCourses"
                color="blue"
                :subtitle="$publishedCourses . ' ' . __('teacher.status_published') . ' • ' . $draftCourses . ' ' . __('teacher.status_draft')"
            >
                <x-slot:icon><x-icon name="book" size="lg" /></x-slot>
            </x-stat-card>

            <x-stat-card
                :label="__('teacher.total_students')"
                :value="number_format($totalStudents)"
                color="emerald"
                :subtitle="__('teacher.students')"
            >
                <x-slot:icon><x-icon name="users" size="lg" /></x-slot>
            </x-stat-card>

            <x-stat-card
                :label="__('teacher.estimated_earnings')"
                :value="number_format($estimatedEarnings)"
                suffix="đ"
                color="indigo"
                value-class="text-blue-600"
                :subtitle="__('teacher.from_paid_orders')"
            >
                <x-slot:icon><x-icon name="currency" size="lg" /></x-slot>
            </x-stat-card>

            <x-stat-card
                :label="__('teacher.pending_courses')"
                :value="$pendingCourses"
                color="amber"
                value-class="text-amber-600"
                :subtitle="__('teacher.pending_admin_approval')"
            >
                <x-slot:icon><x-icon name="clock" size="lg" /></x-slot>
            </x-stat-card>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="flex items-center justify-between pb-4 border-b border-slate-100">
                    <div>
                        <h3 class="text-lg font-black text-slate-900 tracking-tight">
                            {{ __('teacher.recent_enrollments') }}
                        </h3>
                        <p class="text-xs font-medium text-slate-500 mt-0.5">
                            {{ __('teacher.recent_enrollments_desc') }}
                        </p>
                    </div>
                    <a
            ...
            <div class="space-y-6">
                ...
                <div
                    class="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-md relative overflow-hidden"
                >
                    <div class="relative z-10 space-y-3">
                        <span
                            class="px-2.5 py-1 bg-white/20 text-white text-[10px] font-black uppercase tracking-wider rounded-md"
                        >
                        ...
                        <div class="pt-2">
                            <a
                                ...
                                class="inline-flex items-center gap-2 px-4 py-2.5 bg-white text-blue-600 rounded-xl text-xs font-black hover:bg-blue-50 transition-all shadow-sm"
                            >
                ...
                <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-6">
                    <div class="flex items-center justify-between pb-3 border-b border-slate-100">
                        <h3 class="text-sm font-black text-slate-900 tracking-tight">
                            {{ __('teacher.top_performing_courses') }}
                        </h3>
                    </div>
                            ...
                            <div class="py-3 flex items-center justify-between gap-3">
                                <div class="min-w-0">
                                    <a
                                        ...
                                        class="text-xs font-bold text-slate-800 hover:text-blue-600 truncate block"
                                    >
                                        {{ $course->title }}
                                    </a>
                                ...
                                <span
                                    class="px-2.5 py-1 bg-blue-50 text-blue-700 text-xs font-black rounded-lg shrink-0"
                                >
