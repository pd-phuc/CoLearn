...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <x-stat-card :label="__('teacher.total_courses')" :value="$totalCourses" color="blue" :subtitle="$publishedCourses . ' ' . __('teacher.status_published') . ' • ' . $draftCourses . ' ' . __('teacher.status_draft')">
            <x-slot:icon><svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg></x-slot:icon>
        </x-stat-card>

        <x-stat-card :label="__('teacher.total_students')" :value="number_format($totalStudents)" color="emerald" :subtitle="__('teacher.students')">
            <x-slot:icon><svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z" /></svg></x-slot:icon>
        </x-stat-card>
        ...
        <x-stat-card :label="__('teacher.pending_courses')" :value="$pendingCourses" color="amber" value-class="text-amber-600" :subtitle="__('teacher.pending_admin_approval')">
            <x-slot:icon><svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></x-slot:icon>
        </x-stat-card>
    </div>
