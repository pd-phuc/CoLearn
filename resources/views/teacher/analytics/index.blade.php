...
<div class="space-y-8">
    ...
    <div class="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-xs space-y-6">
        ...
        <div class="grid grid-cols-6 sm:grid-cols-12 gap-2 sm:gap-4 items-end h-64 pt-8">
            @php $maxRev = max(max($monthlyRevenue), 1); @endphp
            @foreach($months as $idx => $month)
                ...
                <div class="flex flex-col items-center gap-2 group relative h-full justify-end">
                    {{-- Tooltip --}}
                    <div class="absolute -top-12 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-900 text-white text-[10px] py-1 px-2 rounded-lg pointer-events-none whitespace-nowrap z-20 shadow-md">
                        <p class="font-bold">{{ number_format($rev) }} đ</p>
                        <p class="text-slate-400">{{ $enr }} {{ __('teacher.students_suffix') }}</p>
                    </div>

                    <div class="w-full bg-blue-100 rounded-t-xl overflow-hidden flex flex-col justify-end" style="height: {{ $heightPct }}%">
                        <div class="w-full bg-gradient-to-t from-blue-600 to-indigo-500 h-full rounded-t-xl transition-all group-hover:from-blue-700 group-hover:to-indigo-600"></div>
                    </div>
