...
@section('admin-content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <x-stat-card
            ...
            :value="number_format($stats['revenue']['value'], 0, ',', '.')"
            suffix="đ"
            color="emerald"
            :change="$stats['revenue']['change']"
            :change-label="__('admin.vs_last_month')"
        >
            <x-slot:icon>
        ...
        <x-stat-card
            :label="__('admin.enrollments')"
            :value="$stats['enrollments']['value']"
            color="violet"
            :change="$stats['enrollments']['change']"
            :change-label="__('admin.vs_last_month')"
        >
            <x-slot:icon>
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            @forelse ($topCourses as $course)
                <div class="mb-3 last:mb-0">
                    ...
                    <div class="w-full bg-slate-100 rounded-full h-1.5">
                        <div
                            class="bg-gradient-to-r from-orange-500 to-amber-500 h-1.5 rounded-full"
                            style="width: {{ ($course->enrollments_count / $maxEnrollments) * 100 }}%"
                        ></div>
                    </div>
                </div>
            @empty
            ...
            <div class="space-y-3">
                @forelse ($recentOrders as $order)
                    <a
                        ...
                        class="flex items-center justify-between hover:bg-slate-50 -mx-2 px-2 py-1.5 rounded-lg transition-colors"
                    >
                        <div class="min-w-0">
                        ...
                        <div class="text-right shrink-0 ml-2">
                            ...
                            <span
                                class="inline-block px-1.5 py-0.5 text-[9px] font-bold uppercase rounded {{ $order->status === 'paid' ? 'bg-emerald-100 text-emerald-700' : ($order->status === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600') }}"
                            >
                                {{ $order->status }}
                            </span>
                        </div>
