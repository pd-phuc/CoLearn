        ...
        <div class="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-xs">
            <form class="flex flex-wrap items-center justify-between gap-4" method="GET">
                ...
                <span class="text-xs font-bold text-slate-500">
                    {{ __('messages.showing_courses_count', ['total' => $courses->count()]) }}
                </span>
            </form>
        </div>
                        ...
                        @forelse ($courses as $course)
                            <tr class="hover:bg-slate-50/60 transition-colors">
                                <td class="px-6 py-4">
                                    <div>
                                        <span
                                            class="px-2 py-0.5 bg-orange-50 text-orange-700 text-[10px] font-extrabold uppercase tracking-wider rounded border border-orange-200/60 mb-1 inline-block"
                                        >
                                            {{ $course->category?->name ?? 'General' }}
                                        </span>
                                        <p class="text-sm font-extrabold text-slate-900 line-clamp-1">
                                            {{ $course->title }}
                                        </p>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    @php
                                        $sc = [
                                            ...
                                            'pending_review' => 'bg-amber-50 text-amber-700 border-amber-200/60',
                                            'archived' => 'bg-slate-100 text-slate-400',
                                        ];
                                    @endphp

                                    <span
                                        class="px-2.5 py-1 rounded-full text-xs font-extrabold uppercase tracking-wider border {{ $sc[$course->status] ?? 'bg-slate-100 text-slate-500' }}"
                                    >
