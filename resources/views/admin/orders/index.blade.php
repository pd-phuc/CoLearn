        ...
        <div class="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-xs">
            <form class="flex flex-wrap items-center justify-between gap-4" method="GET">
                ...
                <span class="text-xs font-bold text-slate-500">
                    {{ __('messages.showing_courses_count', ['total' => $orders->count()]) }}
                </span>
            </form>
        </div>
                    ...
                    <tbody class="divide-y divide-slate-100 font-medium">
                        @forelse ($orders as $order)
                            ...
                            <tr class="hover:bg-slate-50/60 transition-colors">
                                <td class="px-6 py-4">
                                    <span
                                        class="px-2.5 py-1 bg-slate-100 text-slate-900 font-mono text-xs font-extrabold rounded-lg border border-slate-200/80"
                                    >
                                        {{ $order->order_number }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="px-2.5 py-1 rounded-full text-xs font-extrabold uppercase tracking-wider border {{ $sc[$order->status] ?? 'bg-slate-100 text-slate-500' }}"
                                    >
                                        {{ $order->status }}
                                    </span>
                                </td>
