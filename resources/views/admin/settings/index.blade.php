...
<div x-data="{ activeTab: '{{ array_key_first($settingGroups) }}' }" class="space-y-6">
    ...
    {{-- Tabs Navigation Bar --}}
    <div class="bg-white border border-slate-200/80 rounded-2xl p-2 shadow-xs flex items-center gap-1 overflow-x-auto">
        @php
            $tabLabels = [
    ...
    <form action="{{ route('admin.settings.update') }}" method="POST" class="space-y-6">
        ...
        @foreach($settingGroups as $groupKey => $group)
            <div x-show="activeTab === '{{ $groupKey }}'" x-transition class="bg-white border border-slate-200/80 rounded-2xl p-6 sm:p-8 shadow-xs space-y-6">

                <div class="border-b border-slate-100 pb-4">
                    <h3 class="text-base font-extrabold text-slate-900">{{ $tabLabels[$groupKey] ?? $group['title'] }}</h3>
                ...
                <div class="pt-6 border-t border-slate-100 flex justify-end">
                    <button type="submit" class="px-6 py-2.5 bg-orange-500 hover:bg-orange-600 text-white font-extrabold text-sm rounded-xl transition-all shadow-sm shadow-orange-500/20 cursor-pointer">
