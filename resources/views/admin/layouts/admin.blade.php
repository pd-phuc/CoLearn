...
<head>
    ...
    <title>{{ isset($title) ? $title . ' — ' . __('admin.panel') : __('admin.panel') . ' — ' . __('messages.app_name') }}</title>
            ...
            <div class="rounded-2xl border border-slate-200/80 bg-white/95 backdrop-blur-xl shadow-md transition-all px-5 py-3 space-y-3">
                ...
                <div class="flex items-center justify-between gap-4 pb-2.5 border-b border-slate-100">
                    ...
                    <div class="flex items-center gap-2.5 shrink-0">
                        <a href="{{ route('admin.dashboard') }}" class="group flex items-center gap-2 transition-all">
                            <div class="relative">
                                <div class="w-9 h-9 rounded-xl bg-gradient-to-tr from-orange-600 to-amber-500 flex items-center justify-center text-white font-black text-xl shadow-md shadow-orange-500/20 group-hover:scale-105 transition-transform duration-200">
                                    C
                                </div>
                        ...
                        <span class="px-2 py-0.5 bg-orange-50 text-orange-600 text-[10px] font-black uppercase tracking-widest rounded-lg border border-orange-200/60 shadow-2xs">
                            Admin
                        </span>
                    </div>
                    ...
                    <div class="flex items-center gap-3.5 shrink-0">
                        {{-- View Site Button --}}
                        <a href="{{ route('home') }}" target="_blank" class="hidden sm:inline-flex items-center gap-2 px-3.5 py-1.5 text-sm font-bold text-slate-700 hover:text-orange-600 hover:bg-orange-50/60 rounded-xl transition-colors">
                            <svg class="w-4.5 h-4.5 text-orange-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                            {{ __('admin.view_site') }}
                        </a>
                        ...
                        <div class="flex items-center bg-slate-100 p-1 rounded-xl text-sm font-bold border border-slate-200/60 shrink-0">
                            <a href="{{ route('lang.switch', 'vi') }}" class="px-3 py-1 rounded-lg transition-all {{ app()->getLocale() === 'vi' ? 'bg-white text-orange-600 shadow-xs' : 'text-slate-500 hover:text-slate-900' }}">VI</a>
                            <a href="{{ route('lang.switch', 'en') }}" class="px-3 py-1 rounded-lg transition-all {{ app()->getLocale() === 'en' ? 'bg-white text-orange-600 shadow-xs' : 'text-slate-500 hover:text-slate-900' }}">EN</a>
                        </div>
                        ...
                        <div x-data="{ open: false }" @click.outside="open = false" class="relative shrink-0">
                            <button @click="open = !open" class="flex items-center gap-2 p-1 rounded-full hover:bg-slate-100 transition-colors focus:outline-none">
                                @if(auth()->user()->avatar)
                                    <img src="{{ auth()->user()->avatar }}" alt="" class="w-9 h-9 rounded-full object-cover ring-2 ring-orange-500/30">
                                @else
                            ...
                            <div x-show="open" x-transition class="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border border-slate-200/80 py-2 z-50 divide-y divide-slate-100" style="display: none;">
                                <div class="px-4 py-3">
                                    ...
                                    <div class="mt-2 flex items-center justify-between">
                                        <span class="px-2.5 py-0.5 bg-purple-100 text-purple-700 font-bold text-[10px] rounded-md uppercase tracking-wider">{{ __('admin.system_admin') }}</span>
                                    </div>
                                </div>
                                <div class="py-1">
                                    <a href="{{ route('admin.profile.edit') }}" class="flex items-center gap-2.5 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-orange-50/80 hover:text-orange-600">
                                    ...
                                    <a href="{{ route('admin.settings.index') }}" class="flex items-center gap-2.5 px-4 py-2 text-sm font-semibold text-purple-700 hover:bg-purple-50">
                        ...
                        @foreach($navItems as $item)
                            ...
                            <a href="{{ route($item['route']) }}"
                               class="flex items-center justify-center gap-2 px-3.5 py-2 rounded-xl text-sm font-bold transition-all whitespace-nowrap {{ $isActive ? 'bg-orange-50 text-orange-600 border border-orange-200/60 shadow-2xs' : 'text-slate-700 hover:text-orange-600 hover:bg-orange-50/60 border border-transparent' }}">
                                <svg class="w-4.5 h-4.5 shrink-0 {{ $isActive ? 'text-orange-500' : 'text-slate-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                ...
                <div x-show="mobileMenuOpen" x-transition class="lg:hidden mt-3 pt-3 border-t border-slate-100 grid grid-cols-2 sm:grid-cols-4 gap-1.5">
                    @foreach($navItems as $item)
                        @php $isActive = request()->routeIs($item['route'] . '*'); @endphp
                        <a href="{{ route($item['route']) }}"
                           class="flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-bold transition-all {{ $isActive ? 'bg-orange-50 text-orange-600 border border-orange-200/60' : 'text-slate-600 hover:bg-slate-50' }}">
                            <svg class="w-4 h-4 shrink-0 {{ $isActive ? 'text-orange-500' : 'text-slate-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="{{ $item['icon'] }}" />
                            </svg>
                            {{ $item['label'] }}
                        </a>
    ...
    <footer class="bg-slate-950 text-slate-400 border-t border-slate-800/80 mt-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 py-14">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-10">
                ...
                <div>
                    <h3 class="text-xs font-extrabold text-white uppercase tracking-wider mb-4">{{ __('admin.admin_nav') }}</h3>
                    <ul class="space-y-2.5 text-sm font-medium">
                        <li><a href="{{ route('admin.dashboard') }}" class="hover:text-orange-400 transition-colors">{{ __('admin.dashboard') }}</a></li>
                        <li><a href="{{ route('admin.users.index') }}" class="hover:text-orange-400 transition-colors">{{ __('admin.users') }}</a></li>
                        <li><a href="{{ route('admin.courses.index') }}" class="hover:text-orange-400 transition-colors">{{ __('admin.courses') }}</a></li>
                        <li><a href="{{ route('admin.settings.index') }}" class="hover:text-orange-400 transition-colors">{{ __('admin.settings') }}</a></li>
                    </ul>
                </div>
