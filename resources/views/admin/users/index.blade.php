    ...
    <div class="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-xs">
        <form class="flex flex-wrap items-center justify-between gap-4" method="GET">
            <div class="flex flex-wrap items-center gap-3 flex-1 min-w-[280px]">
                ...
                <select name="role" onchange="this.form.submit()" class="px-4 py-2.5 bg-slate-50/80 border border-slate-200/80 rounded-xl text-sm font-bold text-slate-700 focus:bg-white focus:border-orange-500 focus:outline-none focus:ring-4 focus:ring-orange-500/10 transition-all">
                    <option value="">{{ __('admin.all_roles') }}</option>
                    <option value="admin" {{ request('role') === 'admin' ? 'selected' : '' }}>{{ __('messages.role_admin') }}</option>
                    <option value="teacher" {{ request('role') === 'teacher' ? 'selected' : '' }}>{{ __('messages.role_teacher') }}</option>
                    <option value="student" {{ request('role') === 'student' ? 'selected' : '' }}>{{ __('messages.role_student') }}</option>
                </select>
            ...
            <span class="text-xs font-bold text-slate-500">{{ __('admin.showing_users_count', ['count' => $users->count()]) }}</span>
        </form>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50/80 border-b border-slate-200/80">
                    <tr>
                        ...
                        <th class="px-6 py-3.5 text-xs font-extrabold text-slate-500 uppercase tracking-wider">{{ __('admin.role') }}</th>
                        <th class="px-6 py-3.5 text-xs font-extrabold text-slate-500 uppercase tracking-wider">{{ __('admin.wallet_balance') }}</th>
                        <th class="px-6 py-3.5 text-xs font-extrabold text-slate-500 uppercase tracking-wider">{{ __('admin.joined_date') }}</th>
                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    @if($user->avatar)
                                        <img src="{{ $user->avatar }}" class="w-9 h-9 rounded-full object-cover ring-2 ring-orange-500/20">
                                    @else
                                        <div class="w-9 h-9 rounded-full bg-gradient-to-tr from-orange-500 to-amber-500 flex items-center justify-center text-white font-black text-xs shadow-xs">
                            ...
                            <td class="px-6 py-4">
                                @foreach($user->roles as $role)
                                    ...
                                    @endphp
                                    <span class="px-2.5 py-0.5 rounded-md text-[10px] font-extrabold uppercase tracking-wider border {{ $roleColors[$role->name] ?? 'bg-slate-100 text-slate-600 border-slate-200' }}">{{ $role->name }}</span>
                                @endforeach
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('admin.users.show', $user) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 hover:bg-orange-50 hover:text-orange-600 text-slate-700 rounded-xl text-xs font-bold transition-colors">
