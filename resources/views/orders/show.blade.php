    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div class="flex items-center justify-between">
            ...
            <a
                ...
                class="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition-colors inline-flex items-center gap-2"
            >
        ...
        @if (session('success'))
            <div
                class="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm font-semibold flex items-center gap-3"
            >
        ...
        <div class="bg-white border border-slate-200/80 rounded-2xl p-6 sm:p-8 shadow-xs space-y-8">
            <!-- Status & Meta Banner -->
            <div
                class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-xl bg-slate-50 border border-slate-200/80"
            >
                <div>
                ...
                <div>
                    ...
                    <p class="text-xs font-extrabold text-slate-900 uppercase mt-1">
            ...
            <div class="space-y-4">
                ...
                <div class="divide-y divide-slate-100 border border-slate-200/80 rounded-xl overflow-hidden">
                    @foreach ($order->items as $item)
                        <div
                            class="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50/50 transition-colors"
                        >
                            <div class="flex items-center gap-4">
                                <div
                                    class="w-16 h-12 rounded-lg bg-slate-100 overflow-hidden shrink-0 border border-slate-200"
                                >
                                    @if ($item->course->thumbnail)
                                        <img
                                            src="{{ asset($item->course->thumbnail) }}"
                                            alt="{{ $item->course->title }}"
                                            class="w-full h-full object-cover"
                                        />
                                    @else
                                        <div
                                            class="w-full h-full bg-gradient-to-br from-orange-500 to-amber-500 flex items-center justify-center text-white font-extrabold text-[10px]"
                                        >
                                            CoLearn
                                        </div>
                                    @endif
                            ...
                            <div class="flex items-center justify-between sm:justify-end gap-4">
                                ...
                                @if ($order->status === 'paid')
                                    <a
                                        href="{{ route('learn.show', $item->course->slug) }}"
                                        class="btn-primary py-2 px-4 rounded-xl text-xs font-bold shadow-xs"
                                    >
