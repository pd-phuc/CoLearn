    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        @if ($orders->isEmpty())
            <div class="bg-white border border-slate-200/80 rounded-2xl p-12 text-center max-w-md mx-auto space-y-4">
                <div
                    class="w-16 h-16 rounded-full bg-slate-100 text-slate-400 mx-auto flex items-center justify-center"
                >
                ...
                <a
                    ...
                    class="btn-primary inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-xs"
                >
            ...
            <div class="bg-white border border-slate-200/80 rounded-2xl overflow-hidden shadow-xs">
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-xs font-semibold text-slate-700">
                        <thead
                            class="bg-slate-50 border-b border-slate-200 text-slate-500 font-extrabold uppercase tracking-wider text-[10px]"
                        >
                            <tr>
                                ...
                                <tr class="hover:bg-slate-50/80 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        @if ($order->status === 'paid')
                                            <span
                                                class="px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-extrabold"
                                            >
                                                {{ __('messages.status_paid') }}
                                            </span>
                                        @elseif ($order->status === 'pending')
                                            <span
                                                class="px-2.5 py-1 rounded-full bg-amber-50 text-amber-700 border border-amber-200 text-[10px] font-extrabold"
                                            >
                                                {{ __('messages.status_pending') }}
                                            </span>
                                        @else
                                            <span
                                                class="px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 border border-slate-200 text-[10px] font-extrabold"
                                            >
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <a
                                            ...
                                            class="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold transition-colors inline-flex items-center gap-1.5"
                                        >
