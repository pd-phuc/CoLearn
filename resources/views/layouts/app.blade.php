...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full bg-slate-100/60">
    <head>
        ...
        <title>
            {{ isset($title) ? $title . ' - ' . __('messages.app_name') : __('messages.app_name') . ' - ' . __('messages.tagline') }}
                    ...
                    <div class="flex items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-6">
                            ...
                            >
                                <button
                                    @click="open = !open"
                                    class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-bold text-slate-700 hover:text-orange-600 hover:bg-orange-50/60 rounded-xl transition-colors"
                                >
                                    <svg
                                ...
                                >
                                    ...
                                    <div
                                        class="px-4 py-1.5 text-[11px] font-bold uppercase tracking-wider text-slate-400"
                                    >
                                        {{ __('messages.categories_menu') }}
                                    </div>
                                    <div class="py-1">
                                        @forelse ($categories as $category)
                                            <a
                                                href="{{ route('courses.index', ['category' => $category->slug]) }}"
                                                class="flex items-center justify-between px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-orange-50/80 hover:text-orange-600 transition-colors"
                                            >
                            ...
                            <nav class="hidden lg:flex items-center gap-1">
                                <a
                                    href="{{ route('home') }}"
                                    class="px-3.5 py-2 text-sm font-bold text-slate-700 hover:text-orange-600 hover:bg-orange-50/60 rounded-xl transition-colors"
                                >
                                    {{ __('messages.home') }}
                                </a>
                                <a
                                    href="{{ route('home') }}#learning-paths"
                                    class="px-3.5 py-2 text-sm font-bold text-slate-700 hover:text-orange-600 hover:bg-orange-50/60 rounded-xl transition-colors"
                                >
                        ...
                        <div class="flex-1 max-w-sm hidden md:block">
                            <form action="{{ route('courses.index') }}" method="GET" class="relative">
                                ...
                                />
                                <svg
                                    class="w-4 h-4 text-slate-400 absolute left-3.5 top-2.5"
                                    fill="none"
                                    viewBox="0 0 24 24"
                                    stroke="currentColor"
                                >
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            >
                                <a
                                    ...
                                    class="px-2.5 py-1 rounded-lg transition-all {{ app()->getLocale() === 'vi' ? 'bg-white text-orange-600 shadow-xs' : 'text-slate-500 hover:text-slate-900' }}"
                                >
                                    VI
                                </a>
                                <a
                                    href="{{ route('lang.switch', 'en') }}"
                                    class="px-2.5 py-1 rounded-lg transition-all {{ app()->getLocale() === 'en' ? 'bg-white text-orange-600 shadow-xs' : 'text-slate-500 hover:text-slate-900' }}"
                                >
                                    EN
                                </a>
                            </div>

                            {{-- Cart Icon with Badge (session-based, visible to all) --}}
                            @inject('cartService', 'App\Services\CartService')
                            <a
                                href="{{ route('cart.index') }}"
                                class="relative hidden sm:flex items-center justify-center w-9 h-9 rounded-xl hover:bg-slate-100 transition-colors"
                            ...
                            @auth
                                ...
                                <a
                                    ...
                                    class="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-orange-50 hover:bg-orange-100 text-orange-700 font-extrabold text-xs rounded-xl border border-orange-200/80 transition-colors shadow-2xs cursor-pointer"
                                >
                                    <svg
                                ...
                                <div x-data="{ open: false }" @click.outside="open = false" class="relative">
                                    ...
                                    >
                                        <div class="px-4 py-3">
                                            ...
                                            <div class="mt-2 flex items-center justify-between">
                                                ...
                                                @elseif (auth()->user()->isTeacher())
                                                    <span
                                                        class="px-2.5 py-0.5 bg-blue-100 text-blue-700 font-bold text-[10px] rounded-md uppercase tracking-wider"
                                                    >
                                                        {{ __('messages.role_teacher') }}
                                                    </span>
                                                @else
                                                    <span
                                                        class="px-2.5 py-0.5 bg-orange-100 text-orange-700 font-bold text-[10px] rounded-md uppercase tracking-wider"
                                                    >
                                        ...
                                        <div class="py-1">
                                            <a
                                                href="{{ route('wallet.index') }}"
                                                class="flex items-center justify-between px-4 py-2 text-sm font-bold text-emerald-700 hover:bg-emerald-50"
                                            >
                                                ...
                                                <span
                                                    class="text-[10px] font-black bg-emerald-100 px-2 py-0.5 rounded-full text-emerald-800"
                                                >
                                                    {{ __('messages.topup_badge') }}
                                                </span>
                                            </a>

                                            <a
                                                href="{{ route('cart.index') }}"
                                                class="flex items-center justify-between px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-orange-50/80 hover:text-orange-600"
                                            >
                                                <span class="flex items-center gap-2">
                                                    <svg
                                                ...
                                                @if ($cartService->count() > 0)
                                                    <span
                                                        class="min-w-[18px] h-[18px] flex items-center justify-center px-1 text-[10px] font-extrabold text-white bg-orange-500 rounded-full"
                                                    >
                                                        {{ $cartService->count() }}
                                                    </span>
                                                @endif
                                            ...
                                            <a
                                                href="{{ route('profile.my-courses') }}"
                                                class="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-orange-50/80 hover:text-orange-600"
                                            >
                                                <svg
                                                    class="w-4 h-4 text-slate-400"
                                                    fill="none"
                            ...
                            @else
                                ...
                                <a
                                    href="{{ route('login') }}"
                                    class="px-3.5 py-2 text-sm font-bold text-slate-700 hover:text-orange-600 transition-colors"
                                >
                                    {{ __('auth.login') }}
                                </a>
