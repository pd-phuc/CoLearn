        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-5 sm:p-6">
            ...
            <div class="space-y-3 md:order-2">
                ...
                @foreach($bankFields as $field)
                    <div class="group hover:border-primary-300 flex items-center justify-between rounded-xl border border-slate-200 bg-white p-3 transition-all">
                        ...
                        @if($field['copy'])
                            <button @click="copyToClipboard('{{ $data[$field['key']] }}')"
                ...
                {{-- Total Amount --}}
                <div class="flex items-center justify-between rounded-xl border-2 border-primary-200 bg-primary-50/50 p-3">
                    <span class="text-[9px] font-black tracking-widest text-primary-600 uppercase">{{ __('messages.total') }}</span>
