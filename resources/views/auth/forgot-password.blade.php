...
<div class="min-h-[70vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-slate-50">
    <div class="max-w-md w-full space-y-8 bg-white p-8 sm:p-10 rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50">
        ...
        <div class="text-center space-y-2">
            <div class="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-orange-500 text-white font-extrabold text-2xl shadow-md shadow-orange-500/20 mb-2">
                C
            </div>
            <h2 class="text-2xl font-black text-slate-900 tracking-tight">{{ __('auth.forgot_password_title') }}</h2>
            <p class="text-xs font-semibold text-slate-500 leading-relaxed">{{ __('auth.forgot_password_sub') }}</p>
        </div>
        ...
        <form action="{{ route('password.email') }}" method="POST" class="mt-6 space-y-5">
            ...
            <div>
                <button type="submit" class="w-full btn-primary py-3.5 text-sm font-bold shadow-md shadow-orange-500/20">
        ...
        <div class="text-center pt-2">
            <a href="{{ route('login') }}" class="inline-flex items-center gap-2 text-xs font-bold text-orange-600 hover:text-orange-700 transition-colors">
