...
<div
    x-data="{
        ...
        openModal(detail) {
            this.videoTitle =
                detail?.title || '{{ __('messages.free_preview_modal_title') }}'
        ...
        >
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-800">
                <div class="flex items-center gap-2">
                    <span
                        class="px-2.5 py-0.5 bg-orange-500/20 text-orange-400 border border-orange-500/30 rounded-md font-bold text-[10px] uppercase tracking-wider"
                    >
                        {{ __('messages.free_preview_modal_title') }}
                    </span>
                    <h3
                        class="font-extrabold text-sm sm:text-base text-white truncate max-w-md"
                        x-text="videoTitle"
                    ></h3>
                </div>
                <button @click="closeModal()" class="text-slate-400 hover:text-white font-bold text-2xl cursor-pointer">
                    &times;
                </button>
            </div>
            ...
            <div class="aspect-video bg-black relative flex items-center justify-center">
                ...
                <template x-if="!videoUrl">
                    <div class="text-center p-8 space-y-3">
                        <div
                            class="w-16 h-16 rounded-full bg-orange-500/20 text-orange-500 flex items-center justify-center mx-auto"
                        >
                            <svg class="w-7 h-7 fill-current text-orange-500 ml-0.5" viewBox="0 0 24 24">
            ...
            <!-- Modal Footer -->
            <div
                class="p-4 bg-slate-950 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-400"
            >
