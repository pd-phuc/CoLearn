...
<div
    x-data="{
        ...
        expandAll() {
            this.openSections = [
                {{ implode(',', range(0, max(0, $course->sections->count() - 1))) }},
            ]
        },
        collapseAll() {
            this.openSections = []
    ...
    <div class="space-y-4">
        @forelse ($course->sections as $sIndex => $section)
            <div class="border border-slate-200/80 rounded-2xl bg-white overflow-hidden shadow-2xs">
                ...
                >
                    <div class="flex items-center gap-3">
                        <svg
                            ...
                            :class="{ 'rotate-180': openSections.includes({{ $sIndex }}) }"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                        >
                ...
                <!-- Lessons List -->
                <div
                    x-show="openSections.includes({{ $sIndex }})"
                    x-transition
                    class="divide-y divide-slate-100 bg-white"
                >
                    @forelse ($section->lessons as $lesson)
                        <div
                            class="px-6 py-3.5 flex items-center justify-between gap-4 hover:bg-orange-50/40 transition-colors"
                        >
                            <div class="flex items-center gap-3 flex-1 min-w-0">
                                ...
                                @if ($lesson->is_free_preview)
                                    <button
                                        @click="$dispatch('open-preview-modal', { videoUrl: '{{ $lesson->video_url }}', title: '{{ $lesson->title }}' })"
