...
<div class="bg-white rounded-3xl border border-slate-200/80 shadow-2xl p-6 space-y-6 sticky top-20">
    ...
    <div class="relative aspect-video rounded-2xl bg-slate-900 overflow-hidden group shadow-md">
        ...
        @else
            <div
                class="w-full h-full bg-gradient-to-tr from-orange-600 via-amber-500 to-orange-400 flex items-center justify-center text-white text-4xl font-black group-hover:scale-105 transition-transform duration-300"
            >
        ...
        @if ($freePreviewCount > 0)
            ...
            <button
                @click="$dispatch('open-preview-modal')"
            ...
            >
                <div
                    class="w-14 h-14 rounded-full bg-orange-500 text-white flex items-center justify-center shadow-xl group-hover:scale-110 transition-transform"
                >
                    <svg class="w-6 h-6 fill-current text-white ml-0.5" viewBox="0 0 24 24">
                        <path d="M8 5v14l11-7z" />
                    </svg>
                </div>
                <span
                    class="mt-2 text-xs font-extrabold uppercase tracking-wider bg-slate-900/80 px-3 py-1 rounded-full backdrop-blur-md"
                >
